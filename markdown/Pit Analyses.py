...
#
# We miight also want to differentiate performance associated with pit stops that include a driver change from pit stops managed by a single driver.

# ## Exploring Pit Event Losses and Gains
#
# If we pit on $l_n$ then we can start to pick apart lap times associated with pit events:
#
...
# - *outlap post-loss*: $l_{n+1}- l_{n+2}$, the delta between the outlap the flying lap following the outlap;
...
#
# As well as looking at simple laptimes, we could go further and start to analyse pit events using the finer grained sector times and driver information. Note however that infomration about what happened during a pit stop (tyre change, refuelling, repairs, etc) is not available.
#
...
#
#
# maybe an opportunity to strat doing some simple supervised deep learning model predictions. For example, can we detect pitsop from eg five consecutive laptimes?
#
